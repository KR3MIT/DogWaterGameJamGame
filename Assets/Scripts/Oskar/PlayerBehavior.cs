...
public class PlayerBehaviour : MonoBehaviour
{
    ...
    public float hardSpeed;// Change by Mads
    ...
    private void Awake()
    {
        ...
        currentSceneName = SceneManager.GetActiveScene().name;// change by mads
    }

    //called every frame
    void Update()
    {
        if (currentSceneName == "Level1")// Change by Mads
        {
        ...
        else if (currentSceneName == "Level3") // change by Mads
        {
            {
                Move = Input.GetAxis("Horizontal");
                rb.velocity = new Vector2(hardSpeed * Move, rb.velocity.y);
                if (Move > offSet || Move < -offSet)
                {
                    Animator.SetBool("isWalking", true);
                    float rotationAngle = Move > 0 ? 0f : (Move < 0 ? 180f : 0f);
